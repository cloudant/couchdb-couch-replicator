...
process_stream_response(ReqId, Worker, HttpDb, Params, Callback) ->
    ...
    {ibrowse_async_headers, ReqId, Code, Headers} ->
        case list_to_integer(Code) of
        429 ->
            Timeout = couch_replicator_ratelimiter:limit(),
...
backoff(HttpDb, Params) ->
    ...
    couch_replicator_ratelimiter:failure(Key),
    throw({retry, HttpDb, Params}).


backoff_success(HttpDb, Params) ->
    Key = backoff_key(HttpDb, Params),
    couch_replicator_ratelimiter:success(Key).


backoff_before_request(Worker, HttpDb, Params) ->
    Key = backoff_key(HttpDb, Params),
    Limit = couch_replicator_ratelimiter:limit(),
    case couch_replicator_ratelimiter:interval(Key) of
