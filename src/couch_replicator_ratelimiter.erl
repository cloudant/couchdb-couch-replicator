...
% This module implements rate limiting based on a variation of the additive
% increase / multiplicative decrease feedback control algorithm.
%
%  https://en.wikipedia.org/wiki/Additive_increase/multiplicative_decrease
%
% This is an adaptive algorithm which converges on the available channel
...
%
% For convenience, the estimated value is an interval (in milliseconds)
% instead of rate. Then the estimated rate = 1000 / interval. So technically
% this implements "additive decrease / multiplicative increase", but it doesn't
%  matter as long as the failure part is multiplicative and the success part is
% additive. If that's the case the algorithm is proven to converge. Otherwise,
% neither multiplicative increase / multiplicative decrease nor additive
% increase / additive decrease will converge.
%
...
%
% Key is any term. Typically would be something like {Method, Url}. The
% result from the function is the current period value. Caller would then
% presumably choose to sleep for that amount of time before or after making
% requests. The current interval can be read with interval(Key) function and
% the max limit with the limit() function.


-module(couch_replicator_ratelimiter).
...
-export([start_link/0]).
-export([interval/1, limit/0, failure/1, success/1]).
...
init([]) ->
    ...
    [ets:new(Tid, Opts) || Tid <- all_tables()],
...
handle_info(cleanup, #state{timer = Timer}) ->
    timer:cancel(Timer),
    [cleanup_table(Tid, now_msec() - ?BACKOFF_LIMIT) || Tid <- all_tables()],
...
update_failure(Key, Interval, _Timestamp, Now) ->
    ...
    Interval3 = erlang:min(Interval2, ?BACKOFF_LIMIT),
...
% it is a step function to help handle larger values as are they are approaching
...
-spec id_to_table(non_neg_integer()) -> atom().
id_to_table(Id) when is_integer(Id) ->
    list_to_atom(atom_to_list(?MODULE) ++ "_" ++ integer_to_list(Id)).


-spec term_to_table(any()) -> atom().
term_to_table(Term) ->
    PHash = erlang:phash2(erlang:term_to_binary(Term)),
    id_to_table(PHash rem ?SHARDS_N).


-spec all_tables() -> [atom()].
all_tables() ->
    [id_to_table(N) || N <- lists:seq(0, ?SHARDS_N - 1)].


-spec new_timer() -> timer:tref().
new_timer() ->
    {ok, Timer} = timer:send_after(?BACKOFF_LIMIT * 2, cleanup),
    Timer.
